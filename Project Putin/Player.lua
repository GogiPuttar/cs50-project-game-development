...
function Player:init(map)
    ...
    self.vicmusic = love.audio.newSource('sounds/w i d e.mp3', 'static')
    self.vicmusic:setVolume(1)
...
function Player:update(dt)
    ...
    if self.x <=  (15 + 7 + 4)*self.map.tileWidth and self.x >=  (15 + 7 + 4 - 1)*self.map.tileWidth and 
        self.y <=  (self.map.mapHeight/2 - 7 + 1)*self.map.tileHeight and self.y >=  (self.map.mapHeight/2 - 7)*self.map.tileHeight then
        victory = 1
        self.sounds['coin']:play()
        self.texture = love.graphics.newImage('graphics/blue alien wide3.png')
        self.width = 48
        self.animations['idle'].frames = {
            love.graphics.newQuad(0, 0, 48, 20, self.texture:getDimensions())
        }
        self.animations['walking'].frames = {
            love.graphics.newQuad(384, 0, 48, 20, self.texture:getDimensions()),
            love.graphics.newQuad(432, 0, 48, 20, self.texture:getDimensions()),
            love.graphics.newQuad(480, 0, 48, 20, self.texture:getDimensions()),
            love.graphics.newQuad(432, 0, 48, 20, self.texture:getDimensions()),
        }
        self.animations['jumping'].frames = {
            love.graphics.newQuad(96, 0, 48, 20, self.texture:getDimensions())
        }
        self.map.music:pause()
        self.vicmusic:setLooping(true)
        self.vicmusic:play()
